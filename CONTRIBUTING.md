...
1. Make commits to your feature branch. Prefix each commit like so:
  ...
  - structure (Adding directories/files for later use)
...
```
git remote add upstream https://github.com/GitMoneyCrew/<NAME_OF_REPO>.git
```
...
Prefix each commit like so
  ...
  ex. [refactor] changed method variables in main.js
...
- The commit message must state the files (or directories) that have been affected.
...
Periodically, when the development branch is in working order, the development branch will be merged into the master branch.
